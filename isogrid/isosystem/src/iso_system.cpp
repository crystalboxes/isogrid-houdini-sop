    ...
    for (int i = 0; i < step; i++)
    {
      for (int j = 0; j < step; j++)
      {
        for (int k = 0; k < step; k++)
        {
          ...
          {
            voxels[index(di + i, dj + j, dk + k)] = false;
            boolean ref = (di + i < I && dj + j < J && dk + k < K) ? buffer[index(di + i, dj + j, dk + k)] : false;
            ...
            if (di + i > 0 && dj + j > 0 && dk + k < K && buffer[index(di + i - 1, dj + j - 1, dk + k)] != ref)
            {
              voxels[index(di + i, dj + j, dk + k)] = true;
              continue;
            }
            if (di + i < I && dj + j > 0 && dk + k > 0 && buffer[index(di + i, dj + j - 1, dk + k - 1)] != ref)
            {
              voxels[index(di + i, dj + j, dk + k)] = true;
              continue;
            }
            if (di + i > 0 && dj + j < J && dk + k > 0 && buffer[index(di + i - 1, dj + j, dk + k - 1)] != ref)
            {
              voxels[index(di + i, dj + j, dk + k)] = true;
              continue;
            }
            if (di + i > 0 && dj + j > 0 && dk + k > 0 && buffer[index(di + i - 1, dj + j - 1, dk + k - 1)] != ref)
            {
              voxels[index(di + i, dj + j, dk + k)] = true;
              continue;
            }
  ...
  for (int i = 0; i < I; i++)
  {
    for (int j = 0; j < J; j++)
    {
      for (int k = 0; k < K; k++)
      {
        ...
        int activeq =
            ((i - 1 >= 0 && buffer[index(i - 1, j, k)]) ? 1 : 0) + ((i + 1 < I && buffer[index(i + 1, j, k)]) ? 1 : 0);
        int activer =
            ((j - 1 >= 0 && buffer[index(i, j - 1, k)]) ? 1 : 0) + ((j + 1 < J && buffer[index(i, j + 1, k)]) ? 1 : 0);
        int actives =
            ((k - 1 >= 0 && buffer[index(i, j, k - 1)]) ? 1 : 0) + ((k + 1 < K && buffer[index(i, j, k + 1)]) ? 1 : 0);
...
void IsoSystem::rotateI()
{
  ...
  for (int i = 0; i < I; i++)
  {
    for (int j = 0; j < J; j++)
    {
      for (int k = 0; k < K; k++)
      {
        voxels[index(i, j, k)] = (J == K) ? buffer[index(i, k, J - 1 - j)] : buffer[index(i, J - 1 - j, K - 1 - k)];
...
void IsoSystem::rotateJ()
{
  ...
  for (int i = 0; i < I; i++)
  {
    for (int j = 0; j < J; j++)
    {
      for (int k = 0; k < K; k++)
      {
        voxels[index(i, j, k)] = (I == K) ? buffer[index(k, j, I - 1 - i)] : buffer[index(I - 1 - i, j, K - 1 - k)];
        colorIndices[index(i, j, k)] =
...
void IsoSystem::rotateK()
{
  ...
  for (int i = 0; i < I; i++)
  {
    for (int j = 0; j < J; j++)
    {
      for (int k = 0; k < K; k++)
      {
        voxels[index(i, j, k)] = (I == J) ? buffer[index(j, I - 1 - i, k)] : buffer[index(I - 1 - i, J - 1 - j, k)];
        colorIndices[index(i, j, k)] =
...
void IsoSystem::drawFaces(int zmin, int zmax)
{
  ...
  if (h.FADE)
  {
    grid->drawFragments(home, colors, zmin, zmax, 5 * (I + J + K) / 16 + h.offset, -(I + J + K) / 4 + h.offset, -I / 2,
                        I / 2, -J / 2, J / 2, -K / 2, K / 2);
  }
  else
  {
