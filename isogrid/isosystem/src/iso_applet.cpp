...
void PIsoApplet::createColors()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		for (int l = 0; l < 3; l++)
		{
			dot = ((i > 6) ? 1.0 : 1.0) * std::max(0.f, normals[i][0] * light[l][0] + normals[i][1] * light[l][1] +
			                                                normals[i][2] * light[l][2]);
			red += dot * light[l][3];
			green += dot * light[l][4];
			blue += dot * light[l][5];
		}
...
void PIsoApplet::keyPressed()
{
	...
	else if (key == 'n')
	{
		...
		iso = new IsoSystem(scale, I, J, K, 0, 0, colors, seed, this);
		creation =
		    std::to_string(iso->seed) + "-" + std::to_string(I) + "/" + std::to_string(J) + "/" + std::to_string(K);
		operators = '\n';
	}
	else if (key == 'N')
	{
